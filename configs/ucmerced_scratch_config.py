...
def get_ucmerced_from_scratch_config():
    config = {}

    # --- 1. Dataset & Dataloader ---
    config['data'] = {
        'dataset_type': 'ucmerced',
        'ucmerced_data_root': 'path/to/UC',
        'ucmerced_test_data_root': None,
        'num_classes': 21,
        'img_size': 224,
        'batch_size': 128,
        'val_batch_size': 128,
    ...
    config['training'] = {
        ...
        'scheduler_params': {'T_max': 500, 'eta_min': 1e-6},
        ...
        'experiment_name': 'ucmerced_experiment',
        'max_recent_checkpoints_to_keep': 5,
    }

    # Propagate device/dtype to mamba
    ...
    # Consistency check
    model_params_ref = config['model']['params']
    if model_params_ref['use_cnn_branch'] and model_params_ref['use_mamba_branch']:
        if len(model_params_ref['mamba_stage_block_counts']) != len(model_params_ref['cnn_output_layers']):
            raise ValueError("Config Error: Mamba stage counts and CNN output layers must match.")
